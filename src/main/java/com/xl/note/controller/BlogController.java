...
import com.xl.note.service.EsBlogService;
...
public class BlogController {
	...
	public String listEsBlogs(
		...
		Pageable firstPageable=new PageRequest(pageIndex, pageSize);
		page=esBlogService.listEsBlogs(firstPageable);
		//系统第一次使用，从数据库将数据导入ElasticSearch
		List<EsBlog> esBlogList=page.getContent();
		if(esBlogList.size()==0||esBlogList==null){
			Page<Blog> blogPage=blogService.listBlogs(firstPageable);
			List<Blog> blogList=blogPage.getContent();
			for (Blog item:blogList) {
				EsBlog transfor = esBlogService.transfor(item);
				esBlogService.updateEsBlog(transfor);
			}
		}
